...
void** x86_64SystemVDefault::GetStackArgumentPtr(CRegisters* pRegisters)
{
	return (void **)(pRegisters->m_rsp->GetValue<uintptr_t>() + sizeof(void *));
}
...
void* x86_64SystemVDefault::GetArgumentPtr(unsigned int iIndex, CRegisters* pRegisters)
{
	...
	return (void *) (pRegisters->m_rsp->GetValue<uintptr_t>() + iOffset);
}

void x86_64SystemVDefault::ArgumentPtrChanged(unsigned int iIndex, CRegisters* pRegisters, void* pArgumentPtr)
{
}

void* x86_64SystemVDefault::GetReturnPtr(CRegisters* pRegisters)
{
	if (m_returnType.type == DATA_TYPE_FLOAT || m_returnType.type == DATA_TYPE_DOUBLE)
		return pRegisters->m_xmm0->m_pAddress; // TODO: Handle 128-bit return values in xmm0 and xmm1

	if (m_pReturnBuffer)
	{
		// First half in rax, second half in rdx
		memcpy(m_pReturnBuffer, pRegisters->m_rax, 8);
		memcpy((void *) ((uintptr_t) m_pReturnBuffer + 8), pRegisters->m_rdx, 8);
...
void x86_64SystemVDefault::ReturnPtrChanged(CRegisters* pRegisters, void* pReturnPtr)
{
	...
	{
		// First half in rax, second half in rdx
		memcpy(pRegisters->m_rax->m_pAddress, m_pReturnBuffer, 8);
		memcpy(pRegisters->m_rdx->m_pAddress, (void *) ((uintptr_t) m_pReturnBuffer + 8), 8);
	}
}
